...
namespace Palette
{
    ...
    MD5::MD5(const string& message)
    {
        ...
        _Init((const byte*)message.c_str(), message.length());
    }
    ...
    const byte* MD5::GetDigest()
    {
        if (!finished) {
            ...
            /* Save number of bits */
            _Encode(count, bits, 8);

            /* Pad out to 56 mod 64. */
            index = (bit32)((count[0] >> 3) & 0x3f);
            padLen = (index < 56) ? (56 - index) : (120 - index);
            _Init(PADDING, padLen);

            /* Append length (before padding) */
            _Init(bits, 8);

            /* Store state in digest */
            _Encode(state, digest, 16);
     ...
     *
     */
    void MD5::_Init(const byte* input, size_t len)
    {
     ...
     */
    void MD5::_Transform(const byte block[64])
    {
    ...
    *
    */
    void MD5::_Encode(const bit32* input, byte* output, size_t length)
    {
     ...
     *
     */
    void MD5::_Decode(const byte* input, bit32* output, size_t length)
    {
        for (size_t i = 0, j = 0; j < length; ++i, j += 4) {
    ...
    string MD5::GetHash(const string& message)
    {
